...
def calculate_actual_values(df: pd.DataFrame, source_config) -> tuple:
    ...
    if source_config.amount_column_name:
        amount_key = source_config.amount_column_name
...
def validate_data_file(
    ...
    if source_config.validation_strategy == "footer":
        ...
        if not companion_path:
            raise ValueError(
                "File validation strategy required a companion file but none was provided."
            )
        ...
        raise ValueError(
            f"Unknown validation strategy: {source_config.validation_strategy}"
        )
    ...
    if db_enabled:
        if connection_string is None:
            raise ValueError(
                "Database connection string is required when db_enabled is True."
            )
        ...
        cursor.execute(
            update_query,
            (expected_count, expected_amount, actual_count, actual_amount, stage_id),
        )
        conn.commit()
        cursor.close()
        conn.close()
