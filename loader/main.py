...
def validate_fields(mapping_config, actual_fields) -> bool:
    ...
    if expected_fields:
        ...
        if missing:
            logger.critical(
                "Expected fields not present in the data file: %s", missing
            )
            return False
    ...
    if expected_positions:
        ...
        if max_position >= actual_count:
            logger.critical(
                "File mapping references %s fields, but only %s are present", max_position, actual_count
            )
            return False
    return True


def run(source_config, file_path, companion_path=None, connection_string=None, file_id=None):
    """
    ...
    reader_kwargs = {
        ...
        'column_specs': source_config.column_specs,
        }
        ...
        if valid_fields:
            ...
            if MSSQL_ENABLED:
                ...
                update_filelog_query = """
                    ...
                    ;
                """
                cursor.execute(update_filelog_query, (stage_id, datetime.now(), "PENDING", file_id))
                stage_id = cursor.fetchone()[0]
                conn.commit()
                cursor.close()
                conn.close()
            ...
            if file_valid:
                logger.info("Record count and amount validation passed for file '%s'", filename)
                ...
                if MSSQL_ENABLED:
                    ...
                    update_filelog_query = """
                        ...
                        ;
                    """
                    cursor.execute(update_filelog_query, (datetime.now(), "PASSED", stage_id))
                    conn.commit()
                    cursor.close()
                    conn.close()
                # Transform the data
                ...
                if MSSQL_ENABLED:
                    ...
                    update_filelog_query = """
                        ...
                        ;
                    """
                    cursor.execute(update_filelog_query, (datetime.now(), "FAILED", stage_id))
                    conn.commit()
                    cursor.close()
                    conn.close()
                logger.critical("Record count and amount validation failed for file '%s'", filename)
                raise ValueError(
                    f"Record count and amount validation failed for file: {filename}"
                )
        else:
            raise ValueError(
                f"Field validation failed for file: {filename}"
            )

if __name__ == "__main__":
    from orchestrator.managers.source_identifier import SourceConfig, load_sources
    sources = load_sources()
    config = SourceConfig('sapar', sources['sapar'])
