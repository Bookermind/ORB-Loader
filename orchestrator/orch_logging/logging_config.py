...
class SQLServerHandler(logging.Handler):
    ...
    def __init__(
        ...
        super().__init__(level=logging.WARNING) # TODO: Does this need moving to CRITICAL?
...
def add_sql_handler(connection_string: str) -> None:
    ...
    """
    root_logger = logging.getLogger()
    formatter = root_logger.handlers[0].formatter if root_logger.handlers else logging.Formatter(
        "%(asctime)s - %(name)s - %(levelname)s - %(message)s",
        datefmt="%Y-%m-%d %H:%M:%S"
    )
